...
void elCleanLivetimes(Int_t run=3085){
  ...
  T->SetBranchAddress("T.shms.pTRIG3_tdcTime", &trig_tdc);
  T->SetBranchAddress("T.shms.pEDTM_tdcTime", &edtm_tdc);
  T->SetBranchAddress("T.shms.pTRIG3_tdcTimeRaw", &trig_tdc_raw);
  ...
  TSP->SetBranchAddress("P.pTRIG3.scaler", &trig_scaler);
  TSP->SetBranchAddress("P.pTRIG3.scalerRate", &trig_scalerRate);
  ...
  TH1D * htrig_raw_cut=new TH1D("htrig_raw_cut","pTRIG3 TDC Time",300,rawmin,rawmax);  TH1D * hdiff_cut=new TH1D("hdiff_cut","Trig2 raw - EDTM raw",300,diffmin,diffmax);
  ...
  */
  ...
  ofstream ofile;
  ofile.open("livetimesCutOnRawDiff189current5.txt",ios::app | ios::out );
 ...
 tx->AddText(string);
 string = Form("Prescale fact:%2.1f",readReport(run,"Ps3 fact"));
 tx->AddText(string);
 tx->Draw();
