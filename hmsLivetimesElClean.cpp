...
void hmsLivetimes(Int_t run=1565){
  Double_t ps=readReport(run,"Ps3 fact");
  //  if(ps<9)return;
  ...
  TString froot = Form("/lustre/expphy/cache/hallc/E12-10-002/abishek/realpass-3b-hms-data/hms_replay_production_%d_-1.root",run);
  ...
  T->SetBranchAddress("T.hms.hTRIG3_tdcTime", &trig_tdc);
  T->SetBranchAddress("T.hms.hEDTM_tdcTime", &edtm_tdc);
  T->SetBranchAddress("T.hms.hTRIG3_tdcTimeRaw", &trig_tdc_raw);
  ...
  TSH->SetBranchAddress("H.hTRIG3.scaler", &trig_scaler);
  TSH->SetBranchAddress("H.hTRIG3.scalerRate", &trig_scalerRate);
  TSH->SetBranchAddress("H.hTRIG1.scalerRate", &trig1_scalerRate);
  TSH->SetBranchAddress("H.EDTM.scaler", &edtm_scaler);
  TSH->SetBranchAddress("H.BCM4C.scalerCurrent", &bcm_scaler);
  TSH->SetBranchAddress("evNumber", &evNumber);
  ...
  TH1D * htrig=new TH1D("htrig","pTRIG3 TDC Time",300,refmin,refmax);
  TH1D * hedtm_raw=new TH1D("hedtm_raw","EDTM RAW TDC Time",300,rawmin,rawmax);
  TH1D * htrig_raw=new TH1D("htrig_raw","pTRIG3 RAW TDC Time",300,rawmin,rawmax);
  TH1D * hdiff=new TH1D("hdiff","Trig3 raw - EDTM raw",300,diffmin,diffmax);

  TH1D * hedtm_cut=new TH1D("hedtm_cut","EDTM TDC Time",300,refmin,refmax);
  TH1D * htrig_cut=new TH1D("htrig_cut","pTRIG3 TDC Time",300,refmin,refmax);
  TH1D * hedtm_raw_cut=new TH1D("hedtm_raw_cut","EDTM TDC Time",300,rawmin,rawmax);
  TH1D * htrig_raw_cut=new TH1D("htrig_raw_cut","pTRIG3 TDC Time",300,rawmin,rawmax);  TH1D * hdiff_cut=new TH1D("hdiff_cut","Trig3 raw - EDTM raw",300,diffmin,diffmax);


  TH1D * htrigcut=new TH1D("htrigcut","pTRIG3 rate",500,0,100000);
  ...
  */
  ...
  ofstream ofile;
  ofile.open("hmsElCleanLivetimes.txt",ios::app | ios::out );
 ...
 tx->AddText(string);
 string = Form("pTRIG3 rate:%4.2f",readReport(run,"pTRIG3 rate"));
 tx->AddText(string);
 string = Form("Prescale fact:%2.1f",readReport(run,"Ps2 fact"));
 tx->AddText(string);
 tx->Draw();
 c1->SaveAs(Form("PDFs/hms/rawDiffrun%d.pdf",run));
