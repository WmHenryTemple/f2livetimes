...
void livetimeRaw(Int_t run=2550){

  Double_t ps=readReport(run,"Ps2 fact");
  //cut on raw times
  //  Double_t edtmHigh=1850;
  ///  Double_t edtmLow=1490;
  //  Double_t trigHigh=100000;//not used;
  //  Double_t trigLow=3250;
  Double_t edtmHigh=9999;
  Double_t edtmLow=0;
  Double_t trigHigh=100000;//not used;
  Double_t trigLow=0;
  ...
  Double_t edtm_scaler, edtm_tdc, trig_scaler, trig_tdc, bcm, bcm_scaler, trig_scalerRate, s1x_scalerRate;  
  TTree *T=(TTree*)f->Get("T");
  T->SetBranchAddress("T.shms.pTRIG2_tdcTimeRaw", &trig_tdc);
  T->SetBranchAddress("T.shms.pEDTM_tdcTimeRaw", &edtm_tdc);
  ...
  TSP->SetBranchAddress("P.S1X.scalerRate", &s1x_scalerRate);
  TSP->SetBranchAddress("P.EDTM.scaler", &edtm_scaler);
  TSP->SetBranchAddress("P.BCM4C.scalerCurrent", &bcm_scaler);

  //declare histos
  TH1D * hedtm=new TH1D("hedtm","EDTM TDC Time Raw",500,1400,2000);
  TH1D * htrig=new TH1D("htrig","pTRIG2 TDC Time",500,3000,4000);
  TH1D * htrigcut=new TH1D("htrigcut","pTRIG2 rate",500,0,100000);
  TH1D * hs1xcut=new TH1D("hs1xcut","S1X rate",500,0,1400000);
  ...
  ofstream ofile;
  ofile.open("livetimesNoTiming.txt",ios::app | ios::out );
  ofile << run <<"\t" << ps <<"\t" << trigRate << "\t"<<s1xRate<<"\t";
