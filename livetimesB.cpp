...
void livetimesB(Int_t run=3204){
  int tightCut=1;
  clock_t t=clock();
  ...
  Double_t edtmHigh=183;
  Double_t edtmLow=148;
  ...
  TH1D * htrig_raw_cut=new TH1D("htrig_raw_cut","pTRIG2 TDC Time",300,rawmin,rawmax);
  ...
  ofstream ofile;
  ofile.open("cut12shmsBoiling.txt",ios::app | ios::out );
 ...
 tx->AddText(string);
 tx->Draw();
 c1->SaveAs(Form("PDFs/cut12run%d.pdf",run));
